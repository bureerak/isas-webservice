...
const { generateToken, verifyToken, requireRole } = require('./auth.js');
...
app.post("/auth/login", async (req, res) => {
    ...
    try {
        const [rows] = await db.master.query(
            "SELECT * FROM Staff WHERE username = ? AND is_active = TRUE", [username]
        );
        ...
        res.json({ token, role: staff.role, full_name: staff.full_name });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});
...
app.get("/auth/me", verifyToken, (req, res) => {
    res.json({ id: req.staff.id, username: req.staff.username, role: req.staff.role, full_name: req.staff.full_name });
});
...
// GET /guests — ดูรายชื่อลูกค้าทั้งหมด
app.get("/guests", verifyToken, requireRole(['manager', 'receptionist']), async (req, res) => {
    try {
        const [rows] = await db.slave.query(
            `SELECT g.*, COUNT(b.id) as total_bookings, MAX(b.check_out_date) as last_stay
             FROM Guests g
...
// GET /guests/:id — ดูโปรไฟล์ลูกค้า + ประวัติการเข้าพัก
app.get("/guests/:id", verifyToken, requireRole(['manager', 'receptionist']), async (req, res) => {
    const { id } = req.params;
    try {
        const [guests] = await db.slave.query("SELECT * FROM Guests WHERE id = ?", [id]);
        if (!guests.length) return res.status(404).json({ error: "Guest not found" });
        const [bookings] = await db.slave.query(
            `SELECT b.*, rt.name as room_type, r.room_number
             FROM Bookings b
             JOIN Rooms r ON b.room_id = r.id
             JOIN RoomTypes rt ON r.room_type_id = rt.id
             WHERE b.guest_id = ?
        ...
        res.json({ ...guests[0], booking_history: bookings });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});

// POST /guests — เพิ่มลูกค้าใหม่
app.post("/guests", verifyToken, requireRole(['manager', 'receptionist']), async (req, res) => {
    const { full_name, phone, email, id_card, nationality, address, notes } = req.body;
    if (!full_name) return res.status(400).json({ error: "full_name is required" });
    try {
        const result = await dbRun(
            "INSERT INTO Guests (full_name, phone, email, id_card, nationality, address, notes) VALUES (?, ?, ?, ?, ?, ?, ?)",
            [full_name, phone || null, email || null, id_card || null, nationality || null, address || null, notes || null]
        );
        res.status(201).json({ guestId: result.insertId, message: "Guest created successfully" });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});
...
// GET /reports/summary — Occupancy + รายได้รายเดือน
app.get("/reports/summary", verifyToken, requireRole('manager'), async (req, res) => {
    try {
        const [revenueByMonth] = await db.slave.query(
            `SELECT
        ...
        const [occupancyToday] = await db.slave.query(
            `SELECT
               (SELECT COUNT(*) FROM Rooms) as total_rooms,
               (SELECT COUNT(DISTINCT room_id) FROM Bookings
                WHERE status = 'Checked_In') as occupied_rooms`
        );

        const [todayArrivals] = await db.slave.query(
        ...
        const [todayDepartures] = await db.slave.query(
            `SELECT b.*, r.room_number, rt.name as room_type
             FROM Bookings b
             JOIN Rooms r ON b.room_id = r.id
             JOIN RoomTypes rt ON r.room_type_id = rt.id
        ...
        res.json({
            ...
            occupancy_today: occupancyToday[0],
