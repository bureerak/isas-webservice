...
/**
 * authFetch — fetch wrapper that automatically attaches JWT token from localStorage
 */
...
if (document.getElementById('bookingForm')) {
    document.getElementById('bookingForm').addEventListener('submit', async (e) => {
        ...
        const bookingData = {
            ...
            guest_phone: document.getElementById('guestPhone') ? document.getElementById('guestPhone').value : '',
            guest_email: document.getElementById('guestEmail') ? document.getElementById('guestEmail').value : '',
...
if (document.getElementById('addRoomForm')) {
    ...
    document.getElementById('addRoomForm').addEventListener('submit', async (e) => {
        ...
        try {
            const response = await authFetch(`${API_BASE}/rooms`, {
                method: 'POST',
...
if (document.getElementById('deleteRoomForm')) {
    document.getElementById('deleteRoomForm').addEventListener('submit', async (e) => {
        ...
        try {
            const response = await authFetch(`${API_BASE}/rooms/${roomId}`, {
                method: 'DELETE'
            });
...
async function loadAllRooms() {
    ...
    tbody.innerHTML = '<tr><td colspan="6" style="text-align: center;"><div class="spinner" style="width:24px; height:24px;"></div></td></tr>';
    ...
    try {
        ...
        if (response.ok && rooms.length > 0) {
            tbody.innerHTML = rooms.map(room => `
                <tr>
                    <td><span style="color:var(--text-muted); font-weight:600;">#${room.id}</span></td>
                    <td style="font-weight:700; color:var(--text-main);">${room.room_number}</td>
                    <td>${room.type_name || 'N/A'}</td>
                    <td><span class="badge-v2 badge-${(room.status || '').toLowerCase()}">${room.status}</span></td>
                    <td style="font-weight:600; color:var(--primary);">฿${room.base_price ? room.base_price.toLocaleString() : 'N/A'}</td>
        ...
        tbody.innerHTML = '<tr><td colspan="6" style="text-align: center; color: var(--danger);">Error loading rooms</td></tr>';
        console.error(error);
    }
}
...
async function loadAllBookings() {
    ...
    try {
        ...
        if (response.ok && bookings.length > 0) {
            tbody.innerHTML = bookings.map(booking => `
                <tr>
                    ...
                    <td><div style="font-weight:700; color:var(--text-main);">${booking.guest_name}</div></td>
                    <td>
                        <div style="font-weight:600;">Room ${booking.room_number}</div>
                        <div style="font-size:0.75rem; color:var(--text-muted);">${booking.room_type || 'N/A'}</div>
                    </td>
                    <td>
                        <div style="font-size:0.875rem;">${formatDate(booking.check_in_date)}</div>
                        <div style="font-size:0.75rem; color:var(--text-muted);">to ${formatDate(booking.check_out_date)}</div>
                    </td>
                    <td style="font-weight:700; color:var(--primary);">฿${booking.total_price.toLocaleString()}</td>
                    <td><span class="badge-v2" style="background: ${getStatusColor(booking.status)}; color: white;">${booking.status}</span></td>
                </tr>
            `).join('');
        } else {
        ...
        tbody.innerHTML = '<tr><td colspan="6" style="text-align: center; color: var(--danger);">Error loading bookings</td></tr>';
        console.error(error);
    }
}
